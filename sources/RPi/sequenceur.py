...
class Machine(Thread):
	...
	def on_message(self,client, userdata, message):
		...
		_compound = self.explode_topic(message.topic)["info"]
		_components = self.composantspour(_compound)
		print(_compound, _components)
		print("publish: {}:{}".format(TOPIC_END,message.payload))
		self.client.publish(topic=TOPIC_END, payload=message.payload, qos=0, retain=False)
		
		if _components != None:	
			for ident in _components:
				print("publish: {}/{}:{}".format(TOPIC_SEARCH_BASE,ident,message.payload))
				self.client.publish(topic=TOPIC_SEARCH_BASE+"/{}".format(ident), payload=message.payload, qos=0, retain=False)
